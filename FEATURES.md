...
- **The Cross-Prompt Encoder (XPE)** — the method from *Cross-Prompt Encoder for
  ...
  → [`models.xpe`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/models/xpe/index.html)
- **XPE, SPT and DUAL are one class**, separated by `encoder_ratio`: `0` is plain soft
  ...
  → [`models.xpe.encoder`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/models/xpe/encoder/index.html)
...
- **One dispatch point** — the `peft` config block routes to stock PEFT (LoRA, prefix
  ...
  → [`models.peft`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/models/peft/index.html)
- **`CustomT5ForConditionalGeneration`** — T5 with optional FlashAttention, pretrained
  q/k/v copied across.
  → [`models.architectures`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/models/architectures/index.html)
...
- **Token-budget batching** — batch by token count rather than row count, with an
  ...
  → [`training.batching`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/training/batching/index.html)
- **Early stopping decoupled from model selection** — HuggingFace ties them together:
  ...
  → [`training.callbacks`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/training/callbacks/index.html)
- **Per-parameter-group optimizer settings from YAML** — give prompt embeddings their
  ...
  → [`custom_training_args.optimizer_grouped_parameters`](https://nlp.micm.edu.ge/en/latest/config.html)
...
- **Collators HuggingFace lacks** — permutation language modelling *with padding*,
  ...
  → [`training.data_collators`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/training/data_collators/index.html)
- **Closed-set generation** — restrict a generative model to a fixed set of allowed
  ...
  → [`training.logits_processors`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/training/logits_processors/index.html)
- **Random task exclusion per batch**, for multi-task training.
  → [`training.trainers`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/training/trainers/index.html)
...
- **Label-restricted likelihood** — score only the candidate label tokens instead of
  ...
  `ds.label.names`.
  → [`evals.eval`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/evals/eval/index.html)
- **Length-normalised log-likelihood accuracy** — without normalisation the shortest
  ...
  → [`evals.metrics.log_likelihood`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/evals/metrics/log_likelihood/index.html)
...
- **Confusion matrices** written per evaluation.
  → [`evals.plot`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/evals/plot/index.html)
...
- **One `DATASET` class over every source** — local CSV / TXT / JSON, the Hub, and
  ...
  → [`datasets.dataset`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/datasets/dataset/index.html)
- **Concatenation across directories** — `get_concatenated_dataset` assembles one
  ...
  → [`DATASET.get_concatenated_dataset`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/datasets/dataset/index.html)
- **Tokenization in three configurable stages** — `tokenize.pre_rules` (sentence
  ...
  → [`DATASET.preprocess`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/datasets/dataset/index.html)
...
- **Length statistics** — `analyze_lengths` reports the real token-length distribution
  ...
  catches.
  → [`DATASET.analyze_lengths`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/datasets/dataset/index.html)
...
- **A factory that knows the architectures** — applies the right special tokens and
  ...
  → [`tokenizers.tokenizer`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/tokenizers/tokenizer/index.html)
...
- **`CustomXlmRoberta`** — XLM-R's multilingual vocabulary re-dressed for another
  architecture.
  → [`tokenizers.architectures`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/tokenizers/architectures/index.html)
- **`KaSenTok`** — a Georgian sentence splitter from *A Comparison of Different
  ...
  → [`tokenizers.ka_sen_tok`](https://nlp.micm.edu.ge/en/latest/autoapi/micm_nlp/tokenizers/ka_sen_tok/index.html)
