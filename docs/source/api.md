...
%% The sidebar's module entries are declared in index.md, grouped into captioned
%% sections. Furo renders captions only for toctrees in the root document, which is
%% why they live there rather than here. This page stays the curated overview.
%% Adding a module means adding it to index.md; the build says so if you forget, via
