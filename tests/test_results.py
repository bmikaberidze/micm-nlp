...
def test_event_name():
    ...
    assert event_name('test', 'after_train') == 'test_after_train'
...
def test_predictions_one_row_per_sample_in_original_order(tmp_path):
    ...
    path = save_predictions(o, 'after_train', _pred_out([1, 0, 1], [1, 1, 1]), cfg, -100, None, None, order=[2, 0, 1])
    assert path == tmp_path / 'run' / 'predictions_after_train.csv'
...
def test_predictions_apply_the_metrics_preprocessing(tmp_path):
    ...
    path = save_predictions(o, 'before_train', _pred_out([1, 0], [0, 0]), cfg, -100, None, None)
    assert path == tmp_path / 'run' / 'separate_predictions_before_train.csv'
...
def test_predictions_token_classification_one_row_per_position(tmp_path):
    ...
    rows = _read(save_predictions(o, 'after_train', out, cfg, -100, None, None))
...
def test_predictions_per_task_join_back_to_the_split(tmp_path):
    ...
    rows = _read(save_predictions(o, 'after_train', _pred_out([1, 0, 1], [1, 1, 1]), cfg, -100, None, split, order=[2, 0, 1]))
